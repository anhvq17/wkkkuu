...
const ReviewManager = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        toast.success("Tải danh sách sản phẩm thành công!", { duration: 2000 });
      } catch (err: unknown) {
        console.error('Lỗi khi lấy danh sách sản phẩm:', err);
        const msg =
          err instanceof Error && 'response' in err
            ? (err as any).response?.data?.message || 'Lỗi khi lấy danh sách sản phẩm'
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        ...
        setUsers(res.data || []);
        toast.success("Tải danh sách người dùng thành công!", { duration: 2000 });
      } catch (err: unknown) {
        console.error('Lỗi khi lấy danh sách người dùng:', err);
        const msg =
          err instanceof Error && 'response' in err
            ? (err as any).response?.data?.message || 'Lỗi khi lấy danh sách người dùng'
    ...
    if (expandedRowKeys.includes(productId)) {
      ...
      if (!reviewsMap[productId]) {
        try {
          ...
          toast.success("Tải đánh giá sản phẩm thành công!", { duration: 2000 });
        } catch (err: unknown) {
          console.error('Lỗi khi lấy đánh giá:', err);
          const msg =
            err instanceof Error && 'response' in err
              ? (err as any).response?.data?.message || 'Lỗi khi lấy đánh giá'
    ...
    if (expandedUserRowKeys.includes(userId)) {
      ...
      if (!userReviewsMap[userId]) {
        try {
          ...
          toast.success("Tải đánh giá người dùng thành công!", { duration: 2000 });
        } catch (err: unknown) {
          console.error('Lỗi khi lấy đánh giá theo người dùng:', err);
          const msg =
            err instanceof Error && 'response' in err
              ? (err as any).response?.data?.message || 'Lỗi khi lấy đánh giá theo người dùng'
              : 'Lỗi khi lấy đánh giá theo người dùng';
          toast.error(msg, { duration: 2000 });
        }
      }
    }
  ...
  const toggleHideReview = async (reviewId: string, productId: string, hidden: boolean | undefined) => {
    const action = hidden ? 'hiện' : 'ẩn';
    const confirmed = await confirmToast(`Bạn có chắc muốn ${action} đánh giá này?`);
    if (!confirmed) return;
    ...
    try {
      ...
      toast.success(`Đã ${action} đánh giá thành công!`, { duration: 2000 });
    } catch (err: unknown) {
      console.error("Lỗi khi cập nhật ẩn/hiện:", err);
      const msg =
        err instanceof Error && 'response' in err
          ? (err as any).response?.data?.message || 'Lỗi khi cập nhật trạng thái ẩn/hiện'
          : 'Lỗi khi cập nhật trạng thái ẩn/hiện';
      toast.error(msg, { duration: 2000 });
    }
  };

  const toggleHideReviewUser = async (reviewId: string, userId: string, hidden: boolean | undefined) => {
    const action = hidden ? 'hiện' : 'ẩn';
    const confirmed = await confirmToast(`Bạn có chắc muốn ${action} đánh giá này?`);
    if (!confirmed) return;
                            ...
                            <tbody>
                              {reviewsMap[product._id].map((r, rIndex) => (
                                <tr key={r._id} className="border-t">
                                  ...
                                  <td className="px-2 py-1">
                                    <button
                                      onClick={() => toggleHideReview(r._id, product._id, r.hidden)}
                            ...
                            <tbody>
                              {userReviewsMap[user._id].map((r, rIndex) => (
                                <tr key={r._id} className="border-t">
                                  ...
                                  <td className="px-2 py-1">
                                    <button
                                      onClick={() => toggleHideReviewUser(r._id, user._id, r.hidden)}
                                      className={`px-3 py-1 rounded-md text-xs text-white ${
                                        r.hidden ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'
                                      }`}
                                    >
